...
exports.userDetail = async (req, res) => {
  try {
    const { profileImg, fullName, address, phone, city } = req.body;

    const user = await User.findById(req.user.id);

    if (!user) return res.status(404).json({ message: "Something Occured!" });
...
exports.updateUserDetails = async (req,res)=>{
  try {
    ...
    res.status(200).json({message: "Update user details successfully"})
